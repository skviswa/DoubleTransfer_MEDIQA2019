...
class MultiTaskBatchSampler(BatchSampler):
    def __init__(self, datasets, batch_size, mix_opt, extra_task_ratio, bin_size=64, bin_on=False, bin_grow_ratio=0.5):
        ...
        for dataset in datasets:
            if bin_on:
                train_data_list.append(self._get_shuffled_index_batches_bin(dataset, batch_size, bin_size=bin_size,
    ...
    def _get_shuffled_index_batches_bin(dataset, batch_size, bin_size, bin_grow_ratio):
        ...
        for idx, sub_data in enumerate(data):
            ...
            sub_batches = [list(range(i, min(i + batch_size, sub_dataset_len))) for i in
                           range(0, sub_dataset_len, batch_size)]
...
class SingleTaskDataset(Dataset):
    ...
    @staticmethod
    def load(path, is_train=True, maxlen=512, factor=1.0, pairwise=False,
             opt=None, dataset='mediqa', printable=True):
